...
# check_min_version("4.42.0.dev0") # this was the initial one
...
class CustomTrainer(Trainer):
    def set_dataset_encoder_and_tokeniser_sampling(
        self,
        datasetEncoder,
    ...
    def init_tokeniser_sampling(
        self,
        device: str,
        file_path: str = "data/uner/avg_seq_len_vs_merges_with_Step_1_for_distr.csv",
        ...
        self.empty_bins = set()
        for bin in self.binned_values:
            if len(self.binned_values[bin]) == 0:
                self.empty_bins.add(bin)
    ...
    def sample_tokeniser_gaussian(self) -> int:
        ...
        sampled_bin_in_empty_bins = True
        while sampled_bin_in_empty_bins:
            sampled_bin = np.random.normal(mean_bin, std_dev_bin, 1)[0]
            sampled_bin = np.clip(np.round(sampled_bin), 0, self.max_bins - 1).astype(
                int
            )
            sampled_bin_in_empty_bins = (sampled_bin + 1) in self.empty_bins
    ...
    def sample_tokeniser_cauchy(self) -> int:
        ...
        while len(sampled_bin_idx) < 1:
            ...
            bin_idx = np.array([sample])[0]
            sampled_bin = np.clip(np.round(bin_idx), 0, self.max_bins - 1).astype(int)
            sampled_bin_in_empty_bins = (sampled_bin + 1) in self.empty_bins

            while sampled_bin_in_empty_bins:
                sample = t.rvs(1, loc=self.max_bins // 2, scale=6)
                bin_idx = np.array([sample])[0]
                sampled_bin = np.clip(np.round(bin_idx), 0, self.max_bins - 1).astype(
                    int
                )
                sampled_bin_in_empty_bins = (sampled_bin + 1) in self.empty_bins
    ...
    def sample_tokeniser_with_oversampling(self) -> int:
        sampled_bin_in_empty_bins = True
        sampled_bin = np.random.choice(
            range(len(self.bin_counts)), size=1, replace=True
        )[0]
        sampled_bin_in_empty_bins = (sampled_bin + 1) in self.empty_bins
        if sampled_bin_in_empty_bins:
            while sampled_bin_in_empty_bins:
    ...
    def sample_tokeniser_cauchy_with_oversampling(self) -> int:
        sampled_bin_idx = []
        while len(sampled_bin_idx) < 1:
            sample = t.rvs(1, loc=self.max_bins // 2, scale=6)
            bin_idx = np.array([sample])[0]
            ...
            if 0 <= sampled_bin <= 29:
                sampled_bin_idx.append(sampled_bin)
    ...
    def sample_tokeniser_student_t_with_oversampling(self) -> int:
        sampled_bin_idx = []
        while len(sampled_bin_idx) < 1:
            sample = t.rvs(10, loc=self.max_bins // 2, scale=6)
            bin_idx = np.array([sample])[0]
    ...
    def _prepare_inputs(
        ...
        if (
            ...
            encoded_batch["labels"] = torch.stack(labels).to(self.device)
        elif (
            ...
            if self.do_tokeniser_sampling_per_batch:  # Uniform
...
class DynamicDataCollator(DefaultDataCollator):
    ...
    def __call__(self, examples):
        ...
        if "inputs_embeds" in examples[0] and "attention_mask" in examples[0]:
            examples = {
                key: torch.stack([item[key] for item in examples])
            ...
            examples["labels"] = (
                []
            )  # Required for trainer.evaluate to compute metrics. Labels are computed in _prepare_inputs() method of CustomTrainer
        return examples
...
def main():
    ...
    if model_args.tokenization_type != "dynamic":
        ...
        try:
            ...
            lang_index = torch.tensor(
                langs.index(data_args.dataset_config_name.split("_")[0]),
                dtype=torch.int32,
            ).to(training_args.device)
    ...
    if (
        ...
        eval_dataset, encoding_time = datasetEncoderValidation.encode_dataset(
        ...
        #     dataset=eval_dataset, batch_size=32, merges=154)
    ...
    if model_args.tokenization_type == "dynamic":
        trainer.set_dataset_encoder_and_tokeniser_sampling(
            datasetEncoder,
